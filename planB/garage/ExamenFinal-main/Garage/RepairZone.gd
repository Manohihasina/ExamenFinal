...
@export var id_reparation_cible : String = "repair_1"
...
func _ready():
	# DEBUG: Version check to ensure code is updated
	print("\n\n🟢 --- REPAIR ZONE V3 (DEBUG) READY ---")
	print("🟢 Zone ID: ", id_reparation_cible)
...
func _process(delta):
	...
	elif joueur_present and not reparation_en_cours and (voiture_id_actuelle == "" or voiture_id_actuelle == "null"):
		if Input.is_action_just_pressed("confirmer"):
...
func reinitialiser_zone():
	...
	# Ne pas cacher le message si le joueur est encore dedans
	# On le mettra à jour via _update_status_message
	if joueur_present:
		_update_status_message()
	else:
		_cacher_message()
