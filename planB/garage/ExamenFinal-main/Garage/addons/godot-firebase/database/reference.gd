...
func on_new_sse_event(headers : Dictionary, event : String, data : Dictionary) -> void:
	if data:
		...
		if command and command != "keep-alive":
			...
			if command == _put_tag:
				if data.path == _separator and data.data:
					if typeof(data.data) == TYPE_DICTIONARY and data.data.keys().size() > 0:
						for key in data.data.keys():
							new_data_update.emit(FirebaseResource.new(_separator + key, data.data[key]))
					elif typeof(data.data) == TYPE_ARRAY and data.data.size() > 0:
						for i in range(data.data.size()):
							if data.data[i] != null:
								new_data_update.emit(FirebaseResource.new(_separator + str(i), data.data[i]))
